...
export const BusinessMetaFields = (props) => {
  const {
    businessId,
    metafields,
    UIComponent,
  ...
  const getMetaFields = async () => {
    try {
      ...
      const response = await fetch(`${ordering.root}/business/${businessId}/metafields`, requestOptions)
  ...
  const handleDeleteMetaField = async (metaFieldId) => {
    try {
      ...
      const response = await fetch(`${ordering.root}/business/${businessId}/metafields/${metaFieldId}`, requestOptions)
      if (response.ok) {
